...
def rms_norm(x: Tensor, normalized_shape: tuple[int, ...], eps: float | None = None) -> Tensor:
...
def load_data_shard(file: Path) -> Tensor:
    ...
    try:
        ...
        # PyTorch 2.1 cannot ingest uint16 directly; int32 preserves the token values.
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        q = rms_norm(q, (q.size(-1),))
        k = rms_norm(k, (k.size(-1),))
...
def main() -> None:
    ...
    if distributed:
        try:
            ...
            # PyTorch 2.1 on the stable Runpod CUDA 11.8 image does not accept device_id.
    ...
    try:
        ...
        def enable_cudnn_sdp(_: bool) -> None:
            return None
